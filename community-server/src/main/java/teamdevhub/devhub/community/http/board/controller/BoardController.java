package teamdevhub.devhub.community.http.board.controller;
...
import teamdevhub.devhub.community.http.board.model.CreateBoardRequestDto;
import teamdevhub.devhub.community.http.board.model.DeleteBoardRequestDto;
...
import teamdevhub.devhub.community.http.board.port.in.Facade.model.BoardDetailResponseDto;
import teamdevhub.devhub.community.http.board.port.in.Facade.model.BoardSummaryResponseDto;
...
public class BoardController {
	...
	public ResponseEntity<DataListApiResponseDto<BoardSummaryResponseDto>> listBoard(@ModelAttribute SearchBoardRequestDto searchBoardRequestDto, PageRequestDto pageRequestDto,
			@LoginUser(required = false) String authenticatedUser) {
		String userGuid = authenticatedUser != null ? authenticatedUser : null;
	...
	@PostMapping
	public ResponseEntity<DataApiResponseDto<Void>> createBoard(@Valid @RequestBody CreateBoardRequestDto createBoardRequestDto, @LoginUser String authenticatedUser) {
		return ResponseEntity.ok(boardFacade.createBoard(createBoardRequestDto.toCommand(authenticatedUser)));
	}
	...
	public ResponseEntity<DataApiResponseDto<BoardDetailResponseDto>> detailBoard(
			...
			HttpServletRequest request, HttpServletResponse response,  @LoginUser(required = false) String authenticatedUser) {
	...
	public ResponseEntity<DataApiResponseDto<Void>> updateBoard(@Valid @RequestBody UpdateBoardRequestDto updateBoardRequestDto, @LoginUser String authenticatedUser,
			@Parameter(description = "게시글 GUID", required = true) @PathVariable("boardGuid") String boardGuid) {
		return ResponseEntity.ok(boardFacade.updateBoard(updateBoardRequestDto.toCommand(authenticatedUser, boardGuid)));
	}
	...
	public ResponseEntity<DataApiResponseDto<Void>> likeBoard(
			@Parameter(description = "게시글 GUID", required = true) @PathVariable("boardGuid") String boardGuid,
			@LoginUser String authenticatedUser) {
		return ResponseEntity.ok(boardFacade.likeBoard(boardGuid, authenticatedUser));
	}
	
	@Operation(summary = "게시글 삭제", description = "게시글 GUID 목록을 받아 일괄 삭제합니다.")
	@ApiResponse(responseCode = "200", description = "삭제 성공")
	@PostMapping("/delete")
	public ResponseEntity<DataApiResponseDto<Void>> deleteBoard(@Valid @RequestBody DeleteBoardRequestDto deleteBoardRequestDto, @LoginUser String authenticatedUser) {
		return ResponseEntity.ok(boardFacade.deleteBoard(deleteBoardRequestDto.getBoardGuid(), authenticatedUser));
	}
}
